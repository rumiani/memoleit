...
export default function Book({ book }: { book: PdfStateTypes }) {  
  ...
  return (
    <div className="relative w-52 h-36 border border-gray-400 rounded-lg p-4 flex flex-col">
      <span className="text-xs font-bold">{timeToNowHandler(book.createdAt).daysHoursAgo}</span>
      <Link href={`/dashboard/docs/${book.id}`}>
        <span title={book.name} className="text-blue-500 p-2 hover:text-blue-700">{book.name}</span>
      </Link>
      <span className="absolute bottom-4 left-4">Size: {formatFileSize(book.size!)}</span>
    </div>
  );
}
